...
Intressantast beträffande återanvändning av kod genom arv är att klassen `PersonalIdentityNumber` ärver från `LuhnCheckDigit`. Lägg märke till hur konstruktorn anropar basklassens konstruktor för att initiera medlemmar deklarerade i basklassen. Värt att uppmärksamma vidare är överskuggning av `IsValid` och `ToString`.